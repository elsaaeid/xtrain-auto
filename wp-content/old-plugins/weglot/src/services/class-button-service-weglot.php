...
class Button_Service_Weglot {
	...
	public function get_html( $add_class = '', $switcher = array(), $add_attr_target = '', $add_attr_sibling = '', $pos = '') {
		...
		// Always output a single input, label (.wgcurrent), and ul as siblings (unified dropdown structure)
		$uniq_id = 'wg' . uniqid( '', true ) . strval( strtotime( 'now' ) ) . wp_rand( 1, 1000 );
		$button_html   .= sprintf( '<input id="%s" class="weglot_choice" type="checkbox" name="menu"/><label data-l="%s" tabindex="-1" for="%s" class="wgcurrent wg-li weglot-lang weglot-language %s" data-code-language="%s" data-name-language="%s"> %s <span class="wg-arrow"></span></label>',
			esc_attr( $uniq_id ),
			esc_attr( $current_language->getExternalCode() ),
			esc_attr( $uniq_id ),
			esc_attr( $flag_class . 'wg-' . $current_language->getInternalCode() ),
			esc_attr( 'wg-' . $current_language->getInternalCode() ),
			esc_html( $name ),
			esc_html( $name )
		);
