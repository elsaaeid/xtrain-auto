...
function _wp_admin_html_begin() {
	...
	$locale = get_locale();
	$rtl_locales = array( 'ar', 'ar_', 'he_', 'fa_', 'ur', 'yi' ); // Arabic, Hebrew, Persian/Farsi, Urdu, Yiddish
	$is_rtl_locale = false;
	
	foreach ( $rtl_locales as $rtl_prefix ) {
		if ( strpos( $locale, $rtl_prefix ) === 0 ) {
			$is_rtl_locale = true;
			break;
		}
	}
	
	$text_direction = $is_rtl_locale ? 'rtl' : 'ltr';

	?>
<!DOCTYPE html>
<html class="<?php echo $admin_html_class; ?>" dir="<?php echo $text_direction; ?>" lang="<?php echo str_replace( '_', '-', get_locale() ); ?>">
<head>
